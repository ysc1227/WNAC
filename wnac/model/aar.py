...
from wnac.nn.basic_var import AdaLNSABlock, SABlock
from wnac.nn.helpers import sample_with_top_k_top_p_, gumbel_softmax_with_rng
from . import WNAC
from wnac.nn.quantize import WavescaleResidualVectorQuantize
...
class AAR(BaseModel):
    def __init__(
        self, vae_local: WNAC, max_patch_size,
        input_dim=512, norm_eps=1e-6, aln=1, aln_gamma_init=1e-3, shared_aln=False, cond_drop_rate=0.1,
        ...
        self.sample_rate = vae_local.sample_rate
        ...
        self.patch_nums: list[int] = [int(max_patch_size * sf) for sf in vae_local.scale_factor[::-1] + vae_local.scale_factor[1:]]
        ...
        self.vae_proxy: Tuple[WNAC] = (vae_local,)
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums):   # si: i-th segment
            ...
            h_BChw = h_BChw.reshape(B, self.vae_quant_proxy[0].codebook_dim, pn)
            f_hat, next_token_map = self.vae_quant_proxy[0].get_next_autoregressive_input(si, f_hat, h_BChw, self.patch_nums[-1])
        ...
        return self.vae_proxy[0].decode(f_hat)   # de-normalize, from [-1, 1] to [0, 1]
