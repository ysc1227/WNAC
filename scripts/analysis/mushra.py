...
MUSHRA_DIR = ROOT / "results" / "mushra"
CACHE_DIR = MUSHRA_DIR / ".cache"
MUSHRA_DIR.mkdir(parents=True, exist_ok=True)
...
# ========= 설정 =========
INPUT_CSV = MUSHRA_DIR / "mushra.csv"
SUMMARY_CSV = MUSHRA_DIR / "mushra_summary.csv"
PLOT_PNG = MUSHRA_DIR / "mushra_dotplot_ci95.png"
PAPER_PLOT_PNG = ROOT / "paper" / "AuthorKit27" / "Figures" / "figure_2_mushra_plot.png"

WNAC_COLOR = "#0072B2"      # proposed WNAC variants
BASELINE_COLOR = "#6C757D"  # DAC/SAT/SNAC baselines
ABLATION_COLOR = "#009E73"  # scale-order ablations
REF_COLOR = "#E69F00"       # anchors/reference
MARKER_SIZE = 6
PLACEHOLDER_CONDITIONS = {
    # Temporary values until matching-bitrate MUSHRA results are collected.
    "DAC (8kbps)": {"mean": 72.0, "ci95": 3.2},
    "WNAC (4.6kbps)": {"mean": 71.5, "ci95": 3.2},
}

# ========= 1. 데이터 로드 =========
df = pd.read_csv(INPUT_CSV)
# df = df[df["email"] == "jessica7050@naver.com"]
df = df[~df["rating_stimulus"].isin({"C3"})].copy()
...
label_map = {
    ...
    "C5": "upscale (8kbps)",
    "C6": "downscale (8kbps)",
    "C7": "WNAC (2.6kbps)",   # SNAC operating point matched by the 2.52 kbps model.
    "reference": "Reference",
    "anchor70": "Anchor 70",
    "anchor35": "Anchor 30",
}
...
placeholder_count = int(summary["count"].max())
for condition, stats in PLACEHOLDER_CONDITIONS.items():
    ci95 = float(stats["ci95"])
    sem = ci95 / 1.96
    summary.loc[condition, ["mean", "std", "count", "sem", "ci95"]] = [
        float(stats["mean"]),
        sem * np.sqrt(placeholder_count),
        placeholder_count,
        sem,
        ci95,
    ]
...
# ========= 4. 플롯에 쓸 순서 지정 =========
# Reference/anchors stay grouped on the left. The model conditions are sorted
# by mean score so quality increases from left to right.
reference_anchor_order = [
    "Reference",
    "Anchor 30",
    "Anchor 70",
]
...
model_order = sorted(
    [cond for cond in model_order if cond in summary.index],
    key=lambda cond: summary.loc[cond, "mean"],
)
order = reference_anchor_order + model_order
...
def condition_color(condition):
    if condition in baseline_models:
        return BASELINE_COLOR
    if condition in wnac_models:
        return WNAC_COLOR
    if condition in ablation_models:
        return ABLATION_COLOR
    return REF_COLOR

colors = [condition_color(cond) for cond in summary.index]
...
legend_elems = [
    ...
    Patch(facecolor=BASELINE_COLOR, edgecolor='black', label='Baselines'),
    Patch(facecolor=ABLATION_COLOR, edgecolor='black', label='Scale-order variants'),
    Patch(facecolor=WNAC_COLOR, edgecolor='black', label='WNAC'),
]
...
plt.savefig(PAPER_PLOT_PNG, dpi=200)

print(f"요약 통계 저장: {SUMMARY_CSV}")
print(f"플롯 저장: {PLOT_PNG}")
print(f"논문 플롯 저장: {PAPER_PLOT_PNG}")
print(summary)


# ========= 7. 사용자(이메일)별 유사도 분석 (모델만 사용) =========
USER_ANALYSIS_CSV = MUSHRA_DIR / "mushra_user_similarity.csv"

# 어떤 컬럼을 사용자 ID로 쓸지 결정 (우선 session_uuid, 없으면 email)
if "session_uuid" in df.columns:
...
if user_col is not None:
    ...
    USER_PLOT_PNG = MUSHRA_DIR / "mushra_user_similarity_plot.png"
