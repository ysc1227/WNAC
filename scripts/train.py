...
Discriminator = argbind.bind(wnac.model.Discriminator)
...
losses = argbind.bind_module(wnac.nn.loss, filter_fn=filter_fn)
...
@dataclass
class State:
    generator: wnac.model.WNAC
    optimizer_g: torch.optim.AdamW
    scheduler_g: ExponentialLR

    discriminator: wnac.model.Discriminator
    optimizer_d: torch.optim.AdamW
    scheduler_d: ExponentialLR

    stft_loss: wnac.nn.loss.MultiScaleSTFTLoss
    mel_loss: wnac.nn.loss.MelSpectrogramLoss
    gan_loss: wnac.nn.loss.GANLoss
    waveform_loss: wnac.nn.loss.L1Loss
...
def load(
    ...
    if resume:
        ...
        if (Path(kwargs["folder"]) / "wnac").exists():
            generator, g_extra = WNAC.load_from_folder(**kwargs)
        if (Path(kwargs["folder"]) / "discriminator").exists():
            discriminator, d_extra = Discriminator.load_from_folder(**kwargs)

    generator = WNAC() if generator is None else generator
